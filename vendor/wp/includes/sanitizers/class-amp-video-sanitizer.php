...
{
	require_once(WEEBLRAMP_AMP__DIR__ . 'includes/sanitizers/class-amp-base-sanitizer.php');
}
	...
	public function sanitize() {
		...
		for ( $i = $num_nodes - 1; $i >= 0; $i-- ) {
			...
			if ( 0 === $new_node->childNodes->length && empty( $new_attributes['src'] ) ) {
				...
				$this->add_script(
					array(
						'amp-video' => sprintf(WeeblrampModel_Renderer::AMP_SCRIPTS_PATTERN, 'video', WeeblrampModel_Renderer::AMP_SCRIPTS_VERSION)
					)
				);
				// weeblr
			}
